...
void test_bal_init_event_type() {
    TEST_ASSERT_EQUAL_MESSAGE(FSM_EVENT_TYPE_IGNORED, balancing_handler.event.type, "Initial event type should be IGNORED");
}

void test_bal_init_cellboard_id() {
    TEST_ASSERT_EQUAL_MESSAGE(CELLBOARD_ID, balancing_handler.status_can_payload.cellboard_id, "Cellboard ID mismatch in payload initialization");
}

void test_bal_init_target() {
    TEST_ASSERT_EQUAL_MESSAGE(BAL_TARGET_MAX_V, balancing_handler.params.target, "Target voltage did not initialize to safe MAX_V");
}

void test_bal_init_threshold() {
    TEST_ASSERT_EQUAL_MESSAGE(BAL_THRESHOLD_MAX_V, balancing_handler.params.threshold, "Threshold voltage did not initialize to safe MAX_V");
}
...
void test_bal_set_balancing_status_handle_target() {
    ...
    TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
        0.001f,
        BAL_TARGET_MIN_V,
        balancing_handler.params.target,
...
void test_bal_set_balancing_status_handle_threshold() {
    ...
    TEST_ASSERT_FLOAT_WITHIN_MESSAGE(delta, BAL_THRESHOLD_MIN_V + delta, balancing_handler.params.threshold, "Threshold voltage was not correctly clamped/validated");
}
...
void test_bal_set_balancing_status_handle_event() {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(FSM_EVENT_TYPE_BALANCING_START, balancing_handler.event.type, "Event type was not updated to BALANCING_START");
}
