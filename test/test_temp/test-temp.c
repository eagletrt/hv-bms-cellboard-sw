...
void test_prv_temp_volt_to_celsius_midrange() {
    ...
    celsius result = prv_temp_api_volt_to_celsius(input);
...
void test_prv_temp_discharge_volt_to_celsius_midrange() {
    ...
    celsius result = prv_temp_api_discharge_volt_to_celsius(input);
...
void test_temp_init_ok() {
    ...
    const enum TempReturnCode rc = temp_api_init(mock_set_address, mock_start_conversion);
...
void test_temp_init_null_set_address() {
    ...
    const enum TempReturnCode rc = temp_api_init(NULL, mock_start_conversion);
...
void test_temp_init_null_start_conversion() {
    ...
    const enum TempReturnCode rc = temp_api_init(mock_set_address, NULL);
...
void test_temp_start_conversion_check_correct_function_calls() {
    const enum TempReturnCode rc = temp_api_start_conversion();
...
void test_temp_notify_conversion_complete_ok() {
    ...
    const enum TempReturnCode rc = temp_api_notify_conversion_complete(raw_values, 2U);
...
void test_temp_update_value_ok() {
    ...
    const enum TempReturnCode rc = temp_api_update_value(3U, expected);
...
void test_temp_update_value_out_of_bounds() {
    const enum TempReturnCode rc = temp_api_update_value(CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT + 1U, 25.0f);
...
void test_temp_update_values_ok() {
    ...
    const enum TempReturnCode rc = temp_api_update_values(3U, values, 2U);
...
void test_temp_update_values_out_of_bounds() {
    ...
    const enum TempReturnCode rc = temp_api_update_values(CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT, values, 1U);
...
void test_temp_update_discharge_value_ok() {
    ...
    celsius expected = prv_temp_api_discharge_volt_to_celsius(2.0f);

    const enum TempReturnCode rc = temp_api_update_discharge_value(3U, 2.0f);
...
void test_temp_update_discharge_value_out_of_bounds() {
    const enum TempReturnCode rc = temp_api_update_discharge_value(CELLBOARD_SEGMENT_DISCHARGE_TEMP_COUNT + 1U, 2.0f);
...
void test_temp_update_discharge_values_with_valid_values() {
    ...
    celsius expected[2] = {
        prv_temp_api_discharge_volt_to_celsius(values[0]),
        prv_temp_api_discharge_volt_to_celsius(values[1])
    };
    const enum TempReturnCode rc = temp_api_update_discharge_values(1U, values, 2U);
...
void test_temp_update_discharge_values_out_of_bounds() {
    ...
    const enum TempReturnCode rc = temp_api_update_discharge_values(CELLBOARD_SEGMENT_DISCHARGE_TEMP_COUNT, values, 1U);
...
void test_temp_get_min() {
    ...
    const celsius min = temp_api_get_min();
...
void test_temp_get_max() {
    ...
    const celsius max = temp_api_get_max();
...
void test_temp_get_sum() {
    ...
    const celsius sum = temp_api_get_sum();
...
void test_temp_get_avg() {
    for (size_t i = 0U; i < CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT; i += 2) {
        temp_handler.temperatures[i] = 10.0f;
        temp_handler.temperatures[i + 1] = 20.0f;
    }

    const celsius avg = temp_api_get_avg();

    TEST_ASSERT_FLOAT_WITHIN_MESSAGE(0.01f, 15.0f, avg, "temp_get_avg() failed");
}

void test_temp_dump_values_null_pointer() {
    const enum TempReturnCode rc = temp_api_dump_values(NULL, 0U, 1U);
...
void test_temp_dump_values_out_of_bounds() {
    ...
    const enum TempReturnCode rc = temp_api_dump_values(out, CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT + 1U, 1U);
...
void test_temp_dump_values_ok() {
    ...
    celsius out[CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT / 2 - 1U] = { 0 };

    celsius expected[CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT / 2 - 1U] = { 0 };
    ...
    const enum TempReturnCode rc = temp_api_dump_values(out, CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT / 2U, CELLBOARD_SEGMENT_TEMP_SENSOR_COUNT / 2U - 1U);
...
void test_temp_get_cells_temp_canlib_payload_values() {
    ...
    size_t size = 0;
    bms_cellboard_cells_temperature_converted_t *payload = temp_api_get_cells_temp_canlib_payload(&size);
...
void test_temp_get_discharge_temp_canlib_payload_values() {
    ...
    size_t size = 0;
    bms_cellboard_discharge_temperature_converted_t *payload = temp_api_get_discharge_temp_canlib_payload(&size);
...
void setUp() {
    ...
    temp_api_init(mock_set_address, mock_start_conversion);
}
